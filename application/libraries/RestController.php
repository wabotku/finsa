...
class RestController extends \CI_Controller
{
    ...
    protected function _log_request($authorized = false)
    {
        ...
        $token = substr($this->_args['Authorization'], 6);
        ...
        $is_inserted = $this->rest->db
            ->insert(
                ...
                [
                    ...
                    'params' => json_encode($this->post_data()),
                    'api_key' => $token,
                    'ip_address' => $this->input->ip_address(),
                    'time' => time(),
     ...
     * @return bool TRUE log table updated; otherwise, FALSE
     */
    protected function _log_response_code($http_code, $data = [])
    {
        if ($this->_insert_id == '') {
            return false;
        }
    ...
    {
        $secret_key = $this->config->item('secretkeyjwt');
        ...
        if ($token) {
            try {
                if($this->basicAuth()){
                    $this->config->set_item('rest_enable_logging', TRUE);

                    return; 
                }

                $this->forwardResponse(self::HTTP_NOT_FOUND, 'failed');
            } catch (\Exception $e) {
                $this->forwardResponse(self::HTTP_FORBIDDEN, 'login back');
            }
        }
    }
    ...
    {
        $this->load->model('v0_1_0/M_Auth');

        $authHeader = isset($this->input->request_headers()['Authorization']) ? $this->input->request_headers()['Authorization'] : false;

        if (!$authHeader) {
            show_404();
        }

        $auth = explode(' ',$authHeader)[0];

        try {
            $encodedCredentials = substr($authHeader, 6);
    
            $token = base64_decode($encodedCredentials);
            $token = str_replace('%20', '+', $token);
            $decodedCredentials = decrypt($token);
            ...
            $res = $this->M_Auth->validate($data);
            
            if (isset($res['status']) && $res['status'] != 1) {
                $this->forwardResponse(self::HTTP_NOT_FOUND, 'Incorrect username or password');
            }
    
            $this->load->library('session');
            $this->session->sess_destroy();
            $this->session->set_userdata('NPTI_ID', $res['data']['IDEMPLOYEE']);
            $this->session->set_userdata('NPTI_IDEMPLOYEE', $res['data']['IDEMPLOYEE']);
        } catch (\Throwable $th) {
            if($auth != 'Basic'){
                return false;
            }

            $this->forwardResponse(self::HTTP_NOT_FOUND, $th->getMessage());
        }

        return true;
    }

    public function forwardResponse($rc = self::HTTP_OK, $rd = 'ok', $data = [])
    {
        $data = [
            'rc' => $rc,
            'rd' => $rd,
        ...
        $this->response($data, $rc);
    }

    public function validator($input, $param, $val = false)
    {
        $invalid = [];
        $invalid_value = [];
        foreach ($param as $value) {
            ...
            if (isset($input[$value]) && $input[$value] == '' && $val) {
                $invalid_value[] = $value;
            }
        }

        if (count($invalid) > 0) {
            $this->forwardResponse($this->HTTP_BAD_REQUEST, 'Required Field', $invalid);
        }

        if (count($invalid_value) > 0) {
            $this->forwardResponse($this->HTTP_BAD_REQUEST, 'Required Value', $invalid_value);
        }
    }
}
